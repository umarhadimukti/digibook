    ...
    <div class="container py-4 mt-2">
      <div class="row1 row">
        ...
        <div class="col-12 col-md-8 wrapper-navigation-2">
          <h5 class="fw-bold mb-3">Ubah Buku {{ $book->title }}</h5>
          <form action="{{ route('dashboard.books.books.update', $book->slug) }}" method="POST" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title"  placeholder="Masukkan Judul Buku.." required value="{{ old('title', $book->title) }}">
              @error('title')
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" placeholder="Masukkan Slug.." required value="{{ old('slug', $book->slug) }}">
              @error('slug')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
            ...
            <div class="mb-3">
              ...
              <select class="form-select @error('category_id') is-invalid @enderror" name="category_id" required>
                ...
                @foreach ($categories as $category)
                  @if (old('category_id', $category->id) == $book->category->id)
            ...
            <div class="mb-3">
              <div class="form-floating">
                <textarea class="form-control @error('description') is-invalid @enderror" placeholder="Deskripsi" id="description" name="description" required style="height: 300px;">{{ old('description', $book->description) }}</textarea>
            ...
            <div class="mb-3">
              ...
              <input type="number" step="0.1" class="form-control @error('rating') is-invalid @enderror" id="rating" name="rating" placeholder="Masukkan Rating Buku.. (*angka)" min="1" max="5" required value="{{ old('rating', $book->rating) }}">
              @error('rating')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
            ...
            <div class="mb-3">
              ...
              <input type="hidden" name="oldbook" value="{{ $book->book }}">
            ...
            <div class="mb-3">
              ...
              @if ($book->cover)
                <img src="{{ asset('storage/' . $book->cover) }}" class="d-block img-fluid img-preview mb-3 col-sm-2 rounded shadow" alt="">
              @else
                <img class="img-fluid img-preview mb-3 col-sm-2 rounded shadow" alt="">
              @endif
              <input type="file" class="form-control @error('cover') is-invalid @enderror" id="cover" name="cover" placeholder="Masukkan Cover Buku.." required>
              @error('cover')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
            ...
            <button type="submit" class="btn btn-tambah">Ubah Buku</button>
          </form>
        </div>
      </div>
    </div>
