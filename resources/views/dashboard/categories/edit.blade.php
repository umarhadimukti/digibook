...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="{{ asset('./css/dashboard/categories/edit.css') }}">
    ...
    <div class="container py-4 mt-2">
      <div class="row1 row">
        ...
        <div class="col-12 col-md-8 wrapper-navigation-2">
          <h5 class="fw-bold mb-3">Ubah Kategori</h5>
          <form action="{{ route('dashboard.categories.categories.update', $category->slug) }}" method="POST" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-3">
              <label for="name" class="form-label text-secondary">Nama Kategori</label>
              <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"  placeholder="Masukkan Nama Kategori.." required value="{{ old('name', $category->name) }}">
              @error('name')
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" placeholder="Masukkan Slug.." required value="{{ old('slug', $category->slug) }}">
              @error('slug')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
            ...
            <div class="mb-3">
              ...
              @if ($category->image)
                <img src="{{ asset('storage/' . $category->image) }}" class="d-block img-fluid img-preview mb-3 col-sm-2 rounded shadow" alt="">
              @else
